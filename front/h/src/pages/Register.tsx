...
export default function Register() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    try {
      const payload = { login: name, password };
      await axios.post("http://localhost:8080/reg", payload, {
  ...
  return (
    <section className="register-wrapper">
      ...
      <aside className="register-decor" aria-hidden="true">
        <div className="register-brand">
          <div className="register-brand__text">
            <span className="register-brand__title">РОСАТОМ</span>
            <span className="register-brand__subtitle">Чат поддержки</span>
          </div>
        </div>
      </aside>
      ...
      <div className="register-content">
        <form onSubmit={onSubmit} className="register-form">
          ...
          <div className="register-tabs">
            <Link to="/auth/login" className="tab-link">
              Вход
            </Link>
            <Link to="/auth/register" className="tab-link active">
              Регистрация
            </Link>
          </div>
        </form>
